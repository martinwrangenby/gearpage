...
describe('InstrumentForm component', () => {
  ...
  test('renders form elements', () => {
    ...
    expect(screen.queryByPlaceholderText(/sold price/i)).not.toBeInTheDocument();
  ...
  test('populates form when rendered with instrument data', () => {
    ...
    render(
      <InstrumentForm submitInstrument={submitInstrumentMock} closeModal={closeModalMock} instrument={existingData}/>);
  ...
  test('shows sold price input when sold switch is toggled on', async () => {
    const user = userEvent.setup();
    render(<InstrumentForm submitInstrument={submitInstrumentMock} closeModal={closeModalMock} />);

    await user.click(screen.getByLabelText(/sold/i));

    expect(screen.getByPlaceholderText(/sold price/i)).toBeVisible();
  });

  test('calls submitInstrument when form is submitted with sold toggled on and sold price entered', async () => {
    const user = userEvent.setup();
    render(<InstrumentForm submitInstrument={submitInstrumentMock} closeModal={closeModalMock} />);
    ...
    const soldPriceInput = screen.getByPlaceholderText(/sold price/i);
    await user.clear(soldPriceInput);
    await user.type(soldPriceInput, '1234');
  ...
  test('calls submitInstrument when form is submitted with valid inputs (not sold)', async () => {
    const user = userEvent.setup();
    render(<InstrumentForm submitInstrument={submitInstrumentMock} closeModal={closeModalMock} />);
  ...
  test('when editing a sold instrument, sold price input is shown and prefilled', () => {
    const existingData = {
      type: 'guitar',
      name: 'Sold Guitar',
    ...
    render(
      <InstrumentForm
        submitInstrument={submitInstrumentMock}
    ...
    const soldPriceInput = screen.getByPlaceholderText(/sold price/i);
    expect(soldPriceInput).toBeVisible();
    expect(soldPriceInput).toHaveValue(999);
  });
